...
print "\n\n##### (1) Run feature generation\n\n";
print("perl $H_script/P1_feature_generation_parallel.pl $targetname $seqfile $dir_models $dir_output\n");
$status = system("perl $H_script/P1_feature_generation_parallel.pl $targetname $seqfile $dir_models $dir_output");
...
if(-e "$ALL_scores/feature_dncon2_short-range.$targetname" and -e "$ALL_scores/feature_dncon2_medium-range.$targetname" and -e "$ALL_scores/feature_dncon2_long-range.$targetname")
{
	...
	print("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea23.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat.txt $native_score\n");
	$status = system("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea23.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat.txt $native_score");
	if($status)# if failed, should we use at least one score?
	{
	...
	print("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds.txt $H_script/python_lib/DeepRank_fea23_models/10fold_2nd_stage_optnadam_htsigmoid_hn5   $dir_output/feat_gdt_predict.txt\n");
	$status = system("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds.txt $H_script/python_lib/DeepRank_fea23_models/10fold_2nd_stage_optnadam_htsigmoid_hn5   $dir_output/feat_gdt_predict.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run the prediction for gdt\n";
	}
